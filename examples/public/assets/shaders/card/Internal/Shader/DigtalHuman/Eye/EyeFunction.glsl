...
vec3 calculateEyeColor(Varyings varyings, mat3 tbn, SurfaceData surfaceData)
{
  ...
  //Sclera Texture
  #ifdef MATERIAL_HAS_SCLERA_TEXTURE  
   vec4 scleraColor = texture2DSRGB(material_ScleraTexture, scleraUV);
  ...
  #ifdef MATERIAL_HAS_SCLERA_MASK
   vec3 irismasktex = (texture2DSRGB(material_Scleramask, irisSizeUV)).rgb;
   float uvmask = 1.0 - (texture2DSRGB(material_Scleramask, varyings.uv )).b;
   heighttexture = (texture2DSRGB(material_Scleramask, parallaxUV)).b;
  #else
  ...
  #ifdef MATERIAL_HAS_IRIS_TEXTURE
   parallax = texture2DSRGB(material_IrisTexture, pupilUV - offset);
  #ifndef ENGINE_IS_COLORSPACE_GAMMA
